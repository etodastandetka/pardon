...
## Причины
1. Nginx использует дефолтную конфигурацию вместо кастомной
2. **Конфликт конфигураций** - домен `pardon.su` настроен в нескольких файлах (см. предупреждения в логах: `conflicting server name "pardon.su"`)

**ВАЖНО:** Если в логах есть предупреждения о конфликте, сначала исправьте их (см. FIX_NGINX_CONFLICT.md)!
